...
//===========================================================================
// Std_RestartIntoSafeMode
//===========================================================================
DEF_STD_CMD(StdCmdRestartIntoSafeMode)

StdCmdRestartIntoSafeMode::StdCmdRestartIntoSafeMode()
  :Command("Std_RestartIntoSafeMode")
{
    sGroup        = "Help";
    sMenuText     = QT_TR_NOOP("Restart into safe mode");
    sToolTipText  = QT_TR_NOOP("Restart into safe mode");
    sWhatsThis    = "Std_RestartIntoSafeMode";
    sStatusTip    = QT_TR_NOOP("Restart into safe mode");
    sPixmap       = "view-refresh";
    eType         = 0;
}

void StdCmdRestartIntoSafeMode::activated(int iMsg)
{
    Q_UNUSED(iMsg);

    QMessageBox restartBox;
    restartBox.setIcon(QMessageBox::Warning);
    restartBox.setWindowTitle(QObject::tr("Restart into safe mode"));
...
void CreateStdCommands()
{
    ...
    rcCmdMgr.addCommand(new StdCmdRestartIntoSafeMode());
